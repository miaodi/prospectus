...
\begin{figure}
    ...
    \end{subfigure}
    \caption{The Legendre polynomials and their approximations in $3^{rd}$ order B\'ezier dual space composed of two elements. B\'ezier dual basis cannot duplicate all except constant function.} 
    \label{fig:polynomial_completeness}
\end{figure}

\subsection{Biharmonic problem on a two-patch domain with homogeneous Dirichlet Boundary} \label{sec:two_patch}
...
\begin{figure}[hbt]
	...
	\caption{The computational domain $\Omega$ and the manufactured solution with the property $u=\frac{\partial{u}}{\partial{\mathbf{n}}}=0$ on $\partial{\Omega}$, which are used in Section~\ref{sec:two_patch}.}\label{fig:two_patch_biharmonic_problem}
\end{figure}
\section{Schedule}
\begin{table}[hbt]
